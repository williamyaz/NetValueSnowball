...
class HedgeBt_C(HedgeBt_SOLE):
    ...
    def process_I(self, i, precision):
        
        '''
        计算连续雪球的各雪球的期权价格和delta的核心函数, 用于多进程
        '''

        _optn = copy.deepcopy(self.optn)
        mkt_data = self.mkt_data
        
        data = mkt_data.iloc[i:i + _optn.matu + 1].copy()
        data.reset_index(drop=True, inplace=True)
        if data.empty:
            return pd.DataFrame()
        
        S_0: float = data.loc[0, 'close'] # type: ignore
        _optn.set_Init_Params(S_0)

        data['ID'] = i + 1
        data['S_0'] = S_0
        data['K'] = _optn.k_price
        data['Up_Barrier_Price'] = _optn.u_prices[0]
        data['Down_Barrier_Price'] = _optn.l_price
        data['kots'] = _optn._kots_vec[:len(data)]
        data = data[
            ['ID', 'Date', 'S_0', 'Up_Barrier_Price', 'K',
             'Down_Barrier_Price', 'close', 'sigma', 'kots',
             'rmq', 'f1_close', 'f2_close', 'expiry_date']
        ]
        ...
        for (j, row) in enumerate(data.itertuples()):
            S = getattr(row, 'close', 0.0)
            sigma = getattr(row, 'sigma', 0.0)
            kots = getattr(row, 'kots', [])
            rmq = getattr(row, 'rmq', 0.0)
            T = _optn.matu - j

            if (not knock_in) and (S <= _optn.l_price):
                knock_in = True
            
            _optn.set_Var_Params(S, rmq, sigma, T, knock_in)
            data.loc[j, 'optn_price'] = _optn.get_Valuation() * _optn.notional
            data.loc[j, 'delta'] = _optn.delta(precision) * _optn.notional

            if (kots[0] == 0) and (S >= _optn.u_prices[0]):
                break
            if T == 0:
                break
        
        return data.iloc[:j+1] # type: ignore
    ...
    def get_Optn_Data(
        ...
        # 多进程执行
        results = []
        with ProcessPoolExecutor(max_workers=max(os.cpu_count()-10, 4)) as executor: # type: ignore
            futures = {
                executor.submit(self.process_I, i, precision): i 
        ...
        # 合并
        optn_data = pd.concat(results, axis=0)
        optn_data['Date'] = pd.to_datetime(optn_data['Date'])
        optn_data = optn_data.sort_values(by=['ID', 'Date'], ascending=[True, True])
